...
class MinNormSolver:
    ...
    def _next_point(cur_val, grad, n):
        if not isinstance(cur_val,np.ndarray):
        ...
        if len(tm1[tm1>1e-7]) > 0:
            ...
            if isinstance(temp,np.ndarray):
                ...
                t = torch.min(temp).numpy()
        if len(tm2[tm2>1e-7]) > 0:
            temp2 = tm2[tm2>1e-7]
            if isinstance(temp2, np.ndarray):
                t = min(t, np.min(tm2[tm2>1e-7]))
            else:
                t = min(t,torch.min(tm2[tm2>1e-7])).numpy()
    ...
    def find_min_norm_element(vecs):
        ...
        while iter_count < MinNormSolver.MAX_ITER:
            ...
            if not isinstance(change,np.ndarray):
                change = change.numpy()
...
def gradient_normalizers(grads, losses, normalization_type,weights = None):
    gn = {}
    if normalization_type == 'l2':
        ...
        for t in grads:
            gn[t] = 1.0
    elif normalization_type == 'lwq_weighted':
        for t in grads:
            gn[t] = weights[t] * np.sqrt(np.sum([gr.pow(2).sum().data.cpu() for gr in grads[t]]))
    else:
